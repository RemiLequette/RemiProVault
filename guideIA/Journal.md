...
- [Session 2026-06-17d — W31 plan-editor localStorage chapitre](#session-2026-06-17d--w31-plan-editor-localstorage-chapitre)
- [Session 2026-06-17c — Triage inbox KB + idées ch. 8](#session-2026-06-17c--triage-inbox-kb--idées-ch-8)
- [Session 2026-06-17b — Bug Introduction plan-editor + restructuration GuideIA.md](#session-2026-06-17b--bug-introduction-plan-editor--restructuration-guideiamd)
- [Session 2026-06-17 — Mise au propre de l'introduction](#session-2026-06-17--mise-au-propre-de-lintroduction)
...
## Session 2026-06-29b — Chapitre 3 : révision du plan et rédaction du guide

### Décisions
- `Plan-ch03.md` réécrit : section `#### Objectifs` ajoutée (4 objectifs), `raisonnement` ajouté aux mots-clés, encadré `biais-de-positivite` ajouté, fin du `#### Contenu` restructurée avec arc narratif complet (biais → raisonnement → conclusion)
- `Plan-ch02.md` corrigé : `génération` retiré des mots-clés — c'est une `{{ref:...}}` vers ch03, pas une `{{def:...}}`
- `GuideIA-ch03.md` complété et affiné en plusieurs passes : biais culturels, "je ne sais pas oral", RLHF en prose courte avec renvoi ch15, encadré `biais-de-positivite`, section "Ce que l'interpolation ne fait pas", section "Une IA n'a que des croyances"
- RLHF : décision de ne pas en faire un mot-clé ni un encadré — introduction en prose simple avec renvoi à l'annexe ch15
- Objectifs ch01 : question ouverte, traitée dans une session dédiée ultérieure
- `append-journal.js` créé dans `guideIA-tools/tools/` — le journal est désormais mis à jour via ce script, sans réécriture complète depuis Claude
- `METHODE.md` mis à jour : section `## Journal` documente le nouveau workflow

### Collaboration

**Moment clé — Le "je ne sais pas" oral**

En relisant le brouillon, l'utilisateur a pointé une explication manquante sur le biais de positivité : les textes écrits ne disent presque jamais "je ne sais pas" — c'est quelque chose qu'on apprend à l'oral. Observation simple, mais elle donne au biais une explication concrète et mémorable, bien au-delà de la formulation technique initiale.

**Moment clé — Tempérer le négatif**

Après l'exemple des Noces de Cana, l'utilisateur a senti que le chapitre risquait de laisser une impression trop négative. Décision d'ajouter une phrase de rééquilibrage : l'étendue des connaissances est vertigineuse, la limite est structurelle, pas une tare. C'est le prix à payer pour une machine qui génère plutôt que pour une machine qui raisonne.

**Moment clé — L'exemple Noces de Cana retravaillé**

La version initiale (Joconde dans les Noces de Cana) était inexacte. L'utilisateur l'a corrigée et enrichie : la Joconde à Paris / les Noces de Cana au Louvre / le syllogisme qui résiste. Trois versions proposées, l'option C retenue — "Elle complète, elle n'enchaîne pas."

**Moment clé — La métaphore du Gaffiot**

Ajoutée par l'utilisateur directement dans le texte, conservée telle quelle. Elle cible le bon public, elle est mémorable, et elle prépare le proverbe italien en chute. Un des passages les plus forts du chapitre.

**Moment clé — Le journal réécrit à chaque session**

En clôturant la session, l'utilisateur a remarqué que Claude réécrit Journal.md en entier à chaque fois — coûteux en tokens et risqué. Décision immédiate : créer `append-journal.js`, un script Node.js qui insère les nouvelles entrées sans que Claude n'ait à lire le fichier existant. Méthode mise à jour en conséquence.
...
- `Plan-ch03.md` réécrit : section `#### Objectifs` ajoutée (4 objectifs), `raisonnement` ajouté aux mots-clés, encadré `biais-de-positivite` ajouté, fin du `#### Contenu` restructurée avec arc narratif complet (biais → raisonnement → conclusion)
- `Plan-ch02.md` corrigé : `génération` retiré des mots-clés — c'est une `{{ref:...}}` vers ch03, pas une `{{def:...}}`
- `GuideIA-ch03.md` complété : trois blocs rédigés à la suite du texte existant — biais culturels + "je ne sais pas oral", RLHF en prose courte avec renvoi ch15, encadré `biais-de-positivite`, section "Ce que l'interpolation ne fait pas", section "Une IA n'a que des croyances"
- RLHF : décision de ne pas en faire un mot-clé ni un encadré — introduction en prose simple avec renvoi à l'annexe ch15
- Objectifs ch01 : question ouverte, traitée dans une session dédiée ultérieure
...
En relisant le brouillon, l'utilisateur a pointé une explication manquante sur le biais de positivité : les textes écrits ne disent presque jamais "je ne sais pas" — c'est quelque chose qu'on apprend à l'oral. Observation simple, mais elle donne au biais une explication concrète et mémorable, bien au-delà de la formulation technique initiale. Ajoutée en corps de texte, avant l'encadré.

**Moment clé — Tempérer le négatif**

Après l'exemple des Noces de Cana, l'utilisateur a senti que le chapitre risquait de laisser une impression trop négative sur les capacités de l'IA. Décision d'ajouter une phrase de rééquilibrage : l'étendue des connaissances est vertigineuse, la limite est structurelle, pas une tare. Et c'est un prix à payer inhérent au mécanisme — point mieux développé dans les chapitres sur l'attention.

**Moment clé — L'exemple Noces de Cana retravaillé**

La version initiale (Joconde dans les Noces de Cana) était inexacte. L'utilisateur l'a corrigée et enrichie : la Joconde à Paris / les Noces de Cana au Louvre / le syllogisme qui résiste. Trois versions proposées, l'option C retenue — "Elle complète, elle n'enchaîne pas." Plus incisive, même mordant que la métaphore du Gaffiot qui suit.

**Moment clé — La métaphore du Gaffiot**

Ajoutée par l'utilisateur directement dans le texte, conservée telle quelle. Elle cible le bon public, elle est mémorable, et elle prépare le proverbe italien en chute. Un des passages les plus forts du chapitre — il n'y avait rien à améliorer.

---

## Session 2026-06-29 — Chapitre 2 : révision du plan et rédaction du guide

### Décisions
- `Plan-ch02.md` entièrement réécrit : objectifs ajoutés, mots-clés révisés, figures conservées, deux tables ajoutées (`operateurs`, `LLM`), trois encadrés (`prompt-systeme`, `GPU`, `open-source`), structure narrative refondée (haut vers bas : figure d'abord, puis composants)
- `GuideIA-ch02.md` entièrement réécrit : suit le nouveau plan, part de la figure, descend dans les composants, intègre taxonomie des opérateurs et table des LLM
- Nouveau type de balise `{{def:tab:...}}` décidé pour les tables — à formaliser dans `METHODE.md` et les outils (TODO outillage)
- Trois nuances ajoutées en fin de session : non-exhaustivité de la table LLM (modèles chinois), différenciation par puissance (renvoi ch. suivant), valeur vers l'infrastructure — "les fabricants de pioches"

### Collaboration
- HOC appliqué naturellement : on a révisé le plan avant de toucher au guide, sans mélanger les niveaux.
- La question sur les agents ("en parle-t-on à ce niveau ?") a produit une réponse nette : mention courte avec renvoi, pas de développement. Bon exemple de décision de périmètre tranchée vite.
- Le type `tab` a émergé organiquement en cours de session — les tables existaient dans le plan brouillon, le système de balises ne les couvrait pas. Décision prise en passant, TODO ouvert, on avance.
- "Les fabricants de pioches" — cliché assumé avec autodérision, dans l'esprit du guide. Nvidia en bénéficiaire discret : mémorable et juste.

---

## Session 2026-06-19 — Intégration Obsidian

### Décisions
- Plan-editor abandonné au profit d'Obsidian (split view natif suffisant)
- `Plan-meta.md` — section `## Chapitres` : liens d'ancre markdown remplacés par des liens Obsidian `[[Plan-chNN|Titre]]`
- `_INDEX.md` créé à la racine du vault guideIA : tableau de bord avec liens vers les fichiers projet et vers Plan + GuideIA de chaque chapitre

### Collaboration
- Incident technique : `str_replace` a échoué sur le chemin Windows malgré un listing réussi — contourné via `write_file` avec réécriture complète du fichier. Pas de perte de contenu (fichier relu avant écriture).

---

## Session 2026-06-17d — W31 plan-editor localStorage chapitre

### Décisions
- W31 implémenté : `localStorage['plan-editor:selected-chapter']` sauvegardé dans `selectCh()`, restauré dans `load()` avec validation et fallback sur le premier chapitre si la clé est absente ou invalide.
...
## Session 2026-06-17c — Triage inbox KB + idées ch. 8

### Décisions
- 8 idées `guide-ia` de l'inbox KB triées — toutes promues dans `#### Contenu` du chapitre 8 (Recueil de bonnes pratiques) de `Plan.md`
- Idée Groundhog Day (métaphore "Un jour sans fin") : triée sans action, couverte par le chapitre 9
- 2 idées ajoutées directement par l'auteur : "Donner les blancs" (laisser l'IA parler en premier, référence aux échecs) et WWH (structurer prompts et instructions en Why / What / How)
- Inbox nettoyée : 8 entrées `guide-ia` supprimées, changelog mis à jour
...
## Session 2026-06-17b — Bug Introduction plan-editor + restructuration GuideIA.md

### Décisions
- Bug identifié : le plan-editor affichait la Conclusion à la place de l'Introduction — tous les chapitres de GuideIA.md portaient le numéro `1` (effet de `autoRenumber` sur un fichier déjà désynchronisé)
- GuideIA.md restructuré complètement : Introduction et Conclusion hors-numérotation (`## Introduction`, `## Conclusion`), chapitres 2 à 18 renumérotés, `## Partie 1/2/3` injectées, chapitres 8 à 18 créés vides
- Titre ch. 3 corrigé : "Le modèle et l'orchestrateur" → "Le modèle et l'opérateur" (orchestrateur = terminologie abandonnée)
- `guide-parser.js` — `parseGuide` corrigé : reconnaît `## Introduction` et `## Conclusion` comme clés nommées, ignore `## Partie N`, robuste aux doublons de numéros (premier gagne)
- `plan-editor.html` — `autoRenumber` désactivée au chargement : la numérotation est désormais gérée manuellement
- W4, W5, W6 terminés et déplacés en Done dans Todo.md
- O28 ajouté (Normal) : aligner `buildState` et `renderNav` dans plan-editor.html avec la nouvelle structure hors-numérotation

### Collaboration
- Diagnostic en deux temps : symptôme signalé (mauvais contenu dans le panneau Guide), cause identifiée dans le parser avant toute écriture. Pas d'action précipitée.
- La vraie cause était en couches : `autoRenumber` avait écrasé la numérotation dans GuideIA.md bien avant cette session — le bug dormait depuis le W4 non terminé. Illustration directe du risque d'une opération automatique silencieuse sur des fichiers sources.
- Le parser corrigé est plus robuste que la spec ne le demandait : il gère les cas limites (doublons, titres `##` non reconnus) sans planter silencieusement.

---

## Session 2026-06-17 — Mise au propre de l'introduction

### Décisions
- Introduction de GuideIA.md rédigée à partir du texte brut de l'auteur dans Plan.md
- Voix à la première personne maintenue — choix éditorial délibéré, documenté dans les méta de Plan.md (nouvelle section « Voix narrative »)
- Sophisme du vrai Écossais : mentionné en prose dans l'introduction, développé dans l'encadré `vrai-ecossais` à rédiger (O27)
- Présentation des trois parties : sobre, narrative, sans catalogue plat (O24 résolu)
- Note de périmètre RGPD intégrée en italique (O23 résolu)
- Plan.md ch. 1 : texte brut remplacé par un résumé structuré (arc, encadré, mot-clé fénéant-créatif)
- Todo.md : O23 et O24 en Done, O27 ajouté en High priority

### Collaboration
- Cadrage initial en trois décisions claires (voix autobiographique, encadré Écossais, résumé dans le plan) avant toute écriture — conforme à la méthode.
- Proposition soumise à validation avant écriture : une relecture, un « c'est ok », puis exécution.
- Incident technique notable : `edit_file` a fait une jointure partielle sur l'ancien texte brut du plan (le délimiteur `oldText` s'arrêtait en milieu de paragraphe). Trois passes de correction nécessaires pour nettoyer. Le fichier est propre.
